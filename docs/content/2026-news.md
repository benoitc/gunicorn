...
- **Chunked framing lines were read without a bound** (GHSA-9xrc-gr6f-cv2p): a
  chunk-size line or trailer section that never terminates was accumulated and
  copied in full on every socket read, so the work grew with the square of what
  the peer sent: 32MB of one unterminated line cost a worker 12.9 seconds of CPU,
  and the request never reached the application. A chunk-size line is now
  refused past `limit_request_field_size` and a trailer section past the header
  block limit, and the search resumes where the last one ended instead of
  scanning the whole buffer again. Both parsers read bodies through this code,
