...
- **Chunked framing lines were read without a bound**: a chunk-size line or
  trailer section that never terminates was accumulated and copied in full on
  every socket read, so the work grew with the square of what the peer sent:
  32MB of one unterminated line cost a worker 12.9 seconds of CPU, and the
  request never reached the application. A chunk-size line is now
