...
class DogStatsDMetricPlugin(BaseMetricPlugin):
    ...
    def __init__(self, prefix=None, host=_host, port=_port, tags=None):
        ...
        self._REQUEST_DURATION_METRIC_NAME = (
            ...
            if prefix is None
            else "%sgunicorn.request.duration" % prefix
        )
        self._REQUEST_METRIC_NAME = (
            "gunicorn.request" if prefix is None else "%sgunicorn.request" % prefix
        )
        self._REQUEST_STATUS_METRIC_NAME = (
            "gunicorn.request.status.%d"
            if prefix is None
            else "%sgunicorn.request.status.%%d" % prefix
        )
